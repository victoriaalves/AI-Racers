...
class Controller(controller_template.Controller):
    ...
    def neighbors(self, currentState):
        ...
        for i in range(1, 100):
            christian = []
            for weight in currentState:
                epsilon = uniform(-0.17, 0.17)
                newWeight = weight + epsilon
                christian.append(newWeight)
            N.append(christian)
        return N
    ...
    def learn(self, weights) -> list:
        ...
        currentBestValue = self.run_episode(weights)
        currentState = weights

        while True:
            bestValue = currentBestValue
            print(currentBestValue)
            N = self.neighbors(weights)
            for n in N:
                value = self.run_episode(n)
                if value > currentBestValue:
                    currentBestValue = value
                    currentState = n
                print("value {}".format(value))
            if currentBestValue > bestValue:
                weights = currentState
            else:
                print("ops")
                return currentState
